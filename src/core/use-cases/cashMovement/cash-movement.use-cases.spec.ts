...
describe('CashMovement Use Cases (Unit)', () => {
  ...
  describe('CreateCashMovementUseCase', () => {
    ...
    it('deve lançar ZodError se o DTO for inválido', async () => {
      ...
      await expect(
        createUseCase.execute(invalidDto, MOCK_USER_ID_VALID),
  ...
  describe('FindAllCashMovementUseCase', () => {
    it('deve chamar o repositório para listar todas as movimentações do usuário', async () => {
      ...
      await findAllUseCase.execute(MOCK_USER_ID_VALID, {
        value: 100,
      } as FindAllCashMovementInput);

      expect(mockCashMovementRepo.findAll).toHaveBeenCalledWith(
        MOCK_USER_ID_VALID,
        { value: 100 },
      );
    });
  ...
  describe('DashboardMovementUseCase', () => {
    ...
    it('deve chamar o repositório com a data fornecida', async () => {
      mockCashMovementRepo.dashboardMovement.mockResolvedValue(
        mockDashboardResult,
      );

      await dashboardUseCase.execute(MOCK_USER_ID_VALID, MOCK_DATE_ISO);

      expect(mockCashMovementRepo.dashboardMovement).toHaveBeenCalledWith(
        MOCK_USER_ID_VALID,
        MOCK_DATE_ISO,
      );
    });
    ...
    it('deve chamar o repositório com a data de hoje por padrão se for passada string vazia', async () => {
      ...
      const receivedUserId =
        mockCashMovementRepo.dashboardMovement.mock.calls[0][0];
      const receivedDate =
        mockCashMovementRepo.dashboardMovement.mock.calls[0][1];
    ...
    it('deve lançar ZodError se o userId for inválido (CORRIGIDO)', async () => {
      ...
      await expect(
        dashboardUseCase.execute(MOCK_USER_ID_INVALID, MOCK_DATE_ISO),
      ).rejects.toThrow(ZodError);
